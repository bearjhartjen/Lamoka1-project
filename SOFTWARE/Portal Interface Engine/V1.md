 ...
 * LAMOKA1 DEV BRAIN — Portal Interface Engine v1

 * Bright games menu • Orange portals every 10 • Real fireball
 ...
 *  8    GAMES   (bright orange)

 *  9    SETTINGS (cyan)

 *

 * GAMES LIST (revolving)

 *  0,10,20,30,40  = BACK TO MAIN (same orange)

 *  All other slots = games
...
enum UIState : uint8_t {
  ...
  UI_MODE=0, UI_MENU_MAIN, UI_MENU_GAMES, UI_MENU_SETTINGS,

  UI_BRIGHTNESS, UI_SPEED, UI_INFO, UI_IN_GAME
...
bool isBackPortal(uint8_t idx) {

  return (idx % 10) == 0;      // 0,10,20,30,40
...
/* ---------- FIREBALL BLAST – real fireball ---------- */

class GameFireball : public Game {

public:

  void enter() override {

    score=0; speed=160; pos=9.0f; state=RUN; last=millis();
  ...
  void render() override {
    ...
    // permanent green safe zone on D1
    ...
    if(state==RUN){
      ...
      if(pos < -0.5f){ state=DEAD; return; }



      int head = (int)pos;

      // multi-LED fireball with heat trail
      ...
      for(int t=0;t<5;t++){
        ...
        if(t==0){                     // core – bright white-yellow
      ...
      fill_solid(leds,10,CRGB(0,200,50));

      leds[0]=CRGB(0,255,90);

      if(now-flashT > 200){

        pos=9.0f; state=RUN; last=now;
      ...
      for(uint8_t i=0;i<10;i++)
        ...
        leds[i] = (i<score) ? CRGB(0,55,18) : CRGB(55,0,0);
...
class GameStacker : public Game {

public:

  void enter() override {

    level=0; width=4; pos=0; dir=1; state=0; delayMs=115; last=millis();

  }

  void update(Gesture g) override {

    if((state==2||state==3)&&g==G_TAP) enter();

    if(state==0 && g==G_TAP) place();
  ...
  void render() override {
    ...
    for(uint8_t i=0;i<level;i++)
      ...
      for(uint8_t j=0;j<stackW[i];j++){

        int l=stackP[i]+j;

        if(l>=0&&l<10) leds[l]=CHSV(95+i*11,230,210);

      }

    if(state==0){

      if(millis()-last>=delayMs){

        last=millis(); pos+=dir;

        if(pos<=0){pos=0;dir=1;}

        if(pos+width>=10){pos=10-width;dir=-1;}

      }

      for(uint8_t j=0;j<width;j++) if(pos+j<10) leds[pos+j]=CRGB(0,255,110);

    } else if(state==1){

      for(uint8_t j=0;j<width;j++) if(pos+j<10) leds[pos+j]=CRGB::White;

      if(millis()-flashT>90) state=0;

    } else if(state==2) fill_solid(leds,10,CRGB(60,0,0));

    else fill_rainbow(leds,10,millis()/4,14);

  }

private:

  uint8_t level,width,state; int8_t pos,dir; uint16_t delayMs; uint32_t last,flashT;

  uint8_t stackP[10],stackW[10];

  void place(){

    int L,R;

    if(level==0){L=pos;R=pos+width-1;}

    else{

      int pL=stackP[level-1],pR=pL+stackW[level-1]-1;

      L=pos>pL?pos:pL; R=(pos+width-1)<pR?(pos+width-1):pR;

    }

    if(R<L){state=2;return;}

    stackP[level]=L; stackW[level]=R-L+1; width=stackW[level]; pos=L; level++;

    state=1; flashT=millis();

    if(level>=9){state=3;return;}

    delayMs=115-level*8; if(delayMs<38) delayMs=38;

    dir=entropy.coin()?1:-1; pos=dir>0?0:10-width;
...
class GameCatch : public Game {

public:

  void enter() override { score=0; lives=3; state=0; next=millis()+650; window=850; }

  void update(Gesture g) override {

    if(state==3&&g==G_TAP) enter();

    if(state==1&&g==G_TAP){

      score++; state=2; hitT=millis();

      int nw=850-score*28; window=(nw>320)?nw:320;
  ...
  void render() override {
    ...
    if(state==0){
      ...
      uint8_t b=beatsin8(5,10,28); fill_solid(leds,10,CRGB(0,b,b/3));

      if(now>=next){ target=entropy.range(9); state=1; appear=now; }

    } else if(state==1){

      leds[target]=CRGB(0,255,70);

      if(target>0) leds[target-1]=CRGB(0,50,12);

      if(target<9) leds[target+1]=CRGB(0,50,12);

      if(now-appear>window){ lives--; state=lives?0:3; next=now+450; }

    } else if(state==2){

      fill_solid(leds,10,CRGB(0,200,50));

      if(now-hitT>130){ state=0; next=now+280+entropy.range(350); }

    } else {

      for(uint8_t i=0;i<10;i++) leds[i]=(i<score)?CRGB(0,50,15):CRGB(50,0,0);
...
private:
  ...
  uint8_t state,score,lives,target; uint16_t window;

  uint32_t next,appear,hitT;
...
class GameReaction : public Game {

public:

  void enter() override { state=0; score=0; }

  void update(Gesture g) override {

    if(state==0&&g==G_TAP){ state=1; wait=millis()+500+entropy.range(1600); }

    if(state==1&&g==G_TAP) state=3;

    if(state==2&&g==G_TAP){

      uint32_t d=millis()-appear;

      score=d<220?10:d<380?8:d<550?5:2; state=4;

    }

    if((state==3||state==4)&&g==G_TAP) enter();
  ...
  void render() override {
    ...
    if(state==0) fill_solid(leds,10,CRGB(0,12,6));

    else if(state==1){ if(millis()>=wait){state=2;appear=millis();target=entropy.range(9);} }

    else if(state==2){ leds[target]=CRGB::White; if(millis()-appear>1100) state=3; }

    else if(state==3) fill_solid(leds,10,CRGB(55,0,0));

    else for(uint8_t i=0;i<score&&i<10;i++) leds[i]=CRGB(0,45,12);

  }

private:

  uint8_t state,score,target; uint32_t wait,appear;

};



/* simple high-contrast variants */

class GameVariant : public Game {

public:

  GameVariant(uint8_t id):id(id){}

  void enter() override { score=0; active=true; }

  void update(Gesture g) override {

    if(!active&&g==G_TAP) enter();

    if(active&&g==G_TAP){ score++; if(score>11) active=false; }
  ...
  void render() override {
    ...
    if(!active){
      ...
      for(uint8_t i=0;i<score&&i<10;i++) leds[i]=CRGB(0,50,18);

      return;

    }

    uint8_t p=(millis()/(55+(id%9)*9)+id*2)%10;

    leds[p]=CHSV(id*13,230,255);
 ...
 * REGISTRIES – exactly 50

 * ================================================================ */

GameFireball gFireball;

GameStacker  gStacker;

GameCatch    gCatch;

GameReaction gReaction;



GameVariant v5(5),v6(6),v7(7),v8(8),v9(9),
            ...
            v31(31),v32(32),v33(33),v34(34),v35(35),

            v36(36),v37(37),v38(38),v39(39),

            v41(41),v42(42),v43(43),v44(44),v45(45),

            v46(46),v47(47),v48(48),v49(49);
...
Game* games[NUM_GAMES] = {
  ...
  &v11,&v12,&v13,&v14,&v15,&v16,&v17,&v18,&v19, // 11-19

  nullptr,          // 20 BACK

  &v21,&v22,&v23,&v24,&v25,&v26,&v27,&v28,&v29, // 21-29

  nullptr,          // 30 BACK

  &v31,&v32,&v33,&v34,&v35,&v36,&v37,&v38,&v39, // 31-39

  nullptr,          // 40 BACK

  &v41,&v42,&v43,&v44,&v45,&v46,&v47,&v48,&v49  // 41-49

};



ModeStatus modeStatus; ModeFire modeFire; ModeSaber modeSaber;

ModeRainbow modeRainbow; ModePulse modePulse; ModeEntropy modeEntropy;

ModeBinary modeBinary; ModeAmbient modeAmbient;
...
uint8_t mainCursor=0, gamesCursor=0, settingsCursor=0;

uint8_t currentMainMode=0, currentGame=1;
...
bool inAnyMenu(){
  ...
  return uiState==UI_MENU_MAIN||uiState==UI_MENU_GAMES||uiState==UI_MENU_SETTINGS||
...
void renderMainMenu() {
  ...
  leds[8]=CRGB(255,90,0);          // bright ORANGE games portal
...
/* ---- GAMES MENU – bright + cool ---- */

void renderGamesMenu() {

  // animated deep background

  uint8_t wave = beatsin8(4, 20, 55);

  for(uint8_t i=0;i<10;i++){

    uint8_t idx = (gamesCursor + i) % NUM_GAMES;



    if(isBackPortal(idx)){

      // bright orange back portal

      leds[i] = CRGB(255, 100, 0);

      leds[i].nscale8(beatsin8(6, 180, 255));

    } else {

      // cool moving spectrum for games

      uint8_t h = (idx * 9 + millis()/40) & 0xFF;

      leds[i] = CHSV(h, 200, 70 + wave/2);
  ...
  // strong cursor on the selected (left) LED
...
void handleUI(Gesture g){
  ...
  if(uiState==UI_IN_GAME){
    ...
    if(g==G_LONG_HOLD){ uiState=UI_MENU_GAMES; touchMenu(); return; }

    if(games[currentGame]) games[currentGame]->update(g);
  ...
  if(uiState==UI_MENU_MAIN){
    ...
    if(g==G_HOLD){
      ...
      if(mainCursor==8){ gamesCursor=0; uiState=UI_MENU_GAMES; touchMenu(); }
  ...
  if(uiState==UI_MENU_GAMES){
    ...
    if(g==G_TAP){ gamesCursor=(gamesCursor+1)%NUM_GAMES; return; }

    if(g==G_HOLD){

      if(isBackPortal(gamesCursor)){

        uiState=UI_MENU_MAIN; touchMenu();

      } else {

        currentGame=gamesCursor;

        if(games[currentGame]) games[currentGame]->enter();
...
void renderUI(){
  ...
  switch(uiState){
    ...
    case UI_IN_GAME: if(games[currentGame]) games[currentGame]->render(); break;
