...
/*
 * LAMOKA1 DEV BRAIN — Portal Interface Engine V1.5
 * -------------------------------------------------
 ...
 *
 * USB BOOTSEL:
 *   Selecting USB BOOTSEL reboots the RP2350 directly
 *   into the ROM USB bootloader.
 *
 * No physical unplug/replug is required.
 */
...
void applyBrightness() {
  uint8_t b = globalBrightness < 18 ? 18 : globalBrightness;
...
class EntropyService {
  ...
  void mix() {
    ...
    s ^= d;
    s ^= micros();
    s ^= (digitalRead(COLOR_ENTROPY_PIN) << 23);
...
public:
  ...
  void render() override {
    ...
    uint16_t iv = 28 + 8000 / (globalSpeed + 50);
...
public:
  void enter() override {
    ...
    moveNoise = entropy.next() & 65535;
    lastMove = millis();
    flare = 0;

    for (uint8_t i = 0; i < 10; i++) {
      heat[i] = 20 + entropy.range(30);
    }
  }
  ...
  void render() override {
    ...
    if (now - lastMove > 240 + entropy.range(260)) {
      newTarget();
      lastMove = now;
    }
...
class ModeSaber : public Mode {
  ...
  void launch() {
    hue = entropy.next8();
    sat = 175 + entropy.range(65);
    pattern = entropy.range(6);
    instability = 20 + entropy.range(80);
...
void renderSettingsMenu() {
  ...
  /*
   * Cursor is pink, except for BOOTSEL and shutdown.
   * Those retain their semantic colors.
   */
...
void enterBootsel() {
  /*
   * Turn the LEDs off before leaving the application.
   *
   * reset_usb_boot() is supplied by the RP2350 boot ROM
   * through the Pico SDK runtime.
   *
   * The RP2350 will reboot directly into USB BOOTSEL.
   */
  clearLeds();
  ...
  /*
   * Normally execution never reaches here because
   * reset_usb_boot() resets the RP2350.
   *
   * Keep the loop here as a defensive fallback.
   */
  while (true) {
    delay(1000);
  }
...
void handleUI(Gesture g) {
  ...
  /* VERY LONG HOLD = RETURN TO MAIN MODE */
  if (g == G_VERY_LONG) {
    fill_solid(
      leds,
      10,
  ...
  if (uiState == UI_MENU_MAIN) {
    ...
    if (g == G_HOLD) {
      ...
      else if (mainCursor < MAIN_MODE_COUNT) {
        currentMainMode = mainCursor;
...
void loop() {
  ...
  if (inAnyMenu() &&
      now - menuLastActivity > MENU_TIMEOUT_MS) {
